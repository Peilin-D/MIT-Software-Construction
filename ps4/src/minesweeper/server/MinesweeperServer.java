...
public class MinesweeperServer {
    ...
    private void handleConnection(Socket socket) throws IOException {
        ...
        // print hello message
        String helloMessage = "Welcome to Minesweeper. Players: 1 including you. "
        					+ "Board: " + board.getWidth() + " columns by " + board.getLength() + " rows. "
        					+ "Type 'help' for help.\n";
        out.print(helloMessage);
        out.flush();
        // handle input
...
//            	System.out.println(output);
    ...
    private String handleRequest(String input) {
        ...
        String helpMessage = "Type \"look\" for current board information; "
        				   + "Type \"dig x y\" to dig at (x, y) location;"
        				   + "Type \"flag x y\" to flag (x, y) location;"
        				   + "Type \"deflag x y\" to deflag (x, y) location;"
...
//        	System.out.println(board.toString());
    ...
    public static void runMinesweeperServer(boolean debug, Optional<File> file, int sizeX, int sizeY, int port) throws IOException {
        if (file.isPresent()) {
        	// Read from file and construct board
        	board = Board.fromFile(file.get());
        } else if (sizeX > 0 && sizeY > 0) {
        	// generate random board
        	board = Board.randomBoard(sizeY, sizeX);
        } else {
        	board = Board.randomBoard(DEFAULT_SIZE, DEFAULT_SIZE);
        }
...
/*
Helper class for multi-threaded server
 */